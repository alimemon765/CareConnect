...
const GRAD = 'linear-gradient(135deg,#FF6B9D,#FF4757)';
const inputStyle = {
...
export default function AmbulanceLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user.role !== 'ambulance') { toast.error('Not an ambulance account.'); setForm({ email: '', password: '' }); return; }
  ...
  return (
    <div className="min-h-dvh bg-bg px-5 pt-6 pb-10">
      <Link to="/" className="inline-flex items-center gap-1 text-text-sub text-sm mb-6"><ChevronLeft size={18} /> Back</Link>
      <div className="flex flex-col items-center mt-4 mb-8">
        <div className="w-20 h-20 rounded-full flex items-center justify-center mb-4"
          style={{ background: GRAD, boxShadow: '0 6px 20px rgba(255,71,87,0.35)' }}>
          <span style={{ fontSize: 40 }}>🚑</span>
        </div>
        <h1 className="text-2xl text-text-main" style={{ fontWeight: 800 }}>Welcome back!</h1>
        <p className="text-sm text-text-sub mt-1">Ambulance Portal</p>
      </div>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            ...
            <input type="email" style={inputStyle} placeholder="Email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </div>
          <div className="relative">
            <Lock size={16} color="#9CA3AF" style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)' }} />
            <input type={showPass ? 'text' : 'password'} style={{ ...inputStyle, paddingRight: 48 }} placeholder="Password"
          ...
          <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.97 }}
            style={{ width: '100%', height: 52, borderRadius: 14, background: GRAD, color: '#fff', fontWeight: 600, fontSize: 15, border: 'none', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
